                                    ...
                                    <div className="book-cover" style={{ width: 128, height: 193, backgroundImage: `url(${book.imageLinks.thumbnail})` }}>  
                                       ...
                                       <div className="book-shelf-changer">
                                         ...
                                         <select value={book.shelf}
                                           onChange={event => this.props.onChangeShelf(this.props.books.filter(b=> {return b.id === `${book.id}` } ), event.target.value)}>
                                           <option value="none" disabled>{book.shelf === 'none' ? 'Add' : 'Move'} to...</option>
                                           <option value="currentlyReading">Currently Reading</option>
                                           <option value="wantToRead">Want to Read</option>
                                           <option value="read">Read</option>{book.shelf !== 'none' && <option value="none">None</option>}
                                        </select>
